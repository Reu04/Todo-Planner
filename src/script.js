class Todo {
    ...
    constructor() {
        ...
        this.itemDeleteButton = this.rootElement.querySelector(this.selectors.itemDeleteButton);
    ...
    onNewTaskFormSubmit = (event) => {
        event.preventDefault(); // отменяем перезагрузку страницы при отправки формы
        ...
        if (newTodoItemTitle.trim().length > 0) {
            ...
            this.newTaskInputElement.focus(); // снова ставим курсор в поле
        }
    }
    ...
    onDeleteAllButtonClick = () => {
        const isConfirmed = confirm("Are you sure to want delete all?");
